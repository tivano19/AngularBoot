...
module.exports = function(env) {
  ...
  const sourceMapEnabled = process.env.SOURCE_MAP === '1';
    ...
    const ENV_SPEC = process.argv[7];
    console.log(ENV_SPEC);
    process.env.ENV_SPEC = ENV_SPEC ? ENV_SPEC.substring(11, ENV_SPEC.length) : 'prod';

    const CTX = process.argv[8];
    console.log(CTX);
    process.env.CTX = CTX.substring(6, CTX.length);

    console.log('Prod ENV_SPEC: ' + process.env.ENV_SPEC);
    console.log('Prod CTX:  ' + process.env.CTX);

  const METADATA = Object.assign({}, buildUtils.DEFAULT_METADATA, {
    ENV_SPEC: process.env.ENV_SPEC,
    CTX: process.env.CTX,
    ENV: ENV,
    HMR: false
  });
  ...
  return webpackMerge(commonConfig({ env: ENV, metadata: METADATA }), {
    ...
    output: {
      ...
      path: helpers.root('dist'),
        ...
        publicPath: process.env.CTX.concat('/content/events/'),
